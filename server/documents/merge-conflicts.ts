...
/**
 * Every key of a fact's family: the key and its by-year map, or a headline
 * and its map (annualRevenue = revenueByYear) - the same family factFamily
 * dedupes by, since a stand-alone figure's year is synced into its map.
 */
function familyKeys(factKey: string): string[] {
  const pair = HEADLINE_MAPS.find((p) => p.head === factKey || p.map === factKey);
  if (pair) return [pair.head, pair.map];
  const map = factFamily(factKey);
  return map === factKey ? [factKey, factKey.replace(/ByYear$/, "")] : [factKey, map];
}
...
function statedValues(info: Record<string, unknown>, factKey: string, factYear: string | null | undefined): { onFile: string[]; all: string[] } {
  ...
  for (const k of familyKeys(factKey)) {
    ...
    if (v && typeof v === "object" && !Array.isArray(v)) {
      ...
      // A stand-alone figure's row carries no year, but its year is synced
      // into this map (merge-policy syncFigureIntoMap), where the losing
      // value may now be that year's other value: still stated.
      else if (!factYear) for (const y of Object.values(map)) if (y !== undefined && y !== null && y !== "") all.push(serial(y));
    } else if (v !== undefined && v !== null && v !== "") onFile.push(serial(v));
    const yearAlts = factYear ? [`${k}.${factYear}`] : Object.keys(alts).concat(Object.keys(corr)).filter((a) => a.startsWith(`${k}.`));
    for (const altKey of [k, ...yearAlts]) {
