...
export async function keepOutFor(dealId: string, info: Record<string, unknown> | null | undefined): Promise<KeepOutResult> {
  ...
  // The rules are re-read every time, cache hit or not: a keep-out request
  // the seller made since (the deal's _sellerKeepOut) is no review
  // candidate, so it doesn't change the cache key — it must still hold.
