...
import { sourceLabel, CRITICAL_CONFLICT_RE, headlineNumber, valuesMateriallyDiffer, type SourceConflict } from "./source-context";
...
export function buildReviewInput(deal: Pick<Deal, "extractedInfo">, documents: DocLike[]): string {
  ...
  for (const d of documents) {
    ...
    const bits = ["summary", "keyFacts", "redFlags", "sellerConcerns"]
      ...
      .filter(Boolean);
    if (bits.length) docLines.push(`### ${label} (${d.name})\n${bits.join("\n")}`);
  }
...
/** A value the model reasoned its way to rather than quoted ("suggest ~100 employees based on typical…"). */
const INFERENCE_RE = /\b(suggest|suggests|implies|imply|indicat\w*|based on typical|typical(?:ly)?|would (?:be|mean|indicate)|likely|presumably|estimated from|works out to|back(?:ed)? into|calculated|computed|derived)\b/i;
 ...
 */
export function validateReviewConflicts(raw: unknown, input: string): SourceConflict[] {
  const list = Array.isArray(raw) ? raw : [];
  const inputNumbers = new Set(numbersIn(input));
  ...
  const quotes = (side: { value: string; source: string }) => {
    ...
    return nums.every((n) => (own ? numbersIn(own).includes(n) : inputNumbers.has(n)));
  };
  const years = (s: string): string[] => s.match(/\b(?:19|20)\d{2}\b/g) ?? [];
  // The period a side speaks to: years in its value, else in its document's
  // title ("tax year 2023") — never a call's or email's date.
