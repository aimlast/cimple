...
**retractedFields** — Facts the seller WITHDREW this turn ("take those numbers back", "I was guessing", "scratch that", "keep that out of the book"): list each one's extractedInfo key with the reason, and do not re-record the value. The server removes it from the facts and keeps it out for the rest of the session. Pair it with a newDeferral naming who holds the real answer. Empty on almost every turn.
