...
export function assembleKnowledgeBase(
  ...
  const flaggedRisksAll = buildFlaggedRisks(documents);
  ...
  const rawCoverage = buildSectionCoverage(extractedInfo, confidenceLevels, sectionImportance, outline.excludedSections, baseAdjustments);
  ...
  const sectionCoverage = Object.keys(fieldsOnFile).length > 0
    ? buildSectionCoverage(extractedInfo, confidenceLevels, sectionImportance, outline.excludedSections, { ...baseAdjustments, onFile: fieldsOnFile })
    : rawCoverage;
...
export function renderKnowledgeBaseForPrompt(kb: KnowledgeBase): string {
  ...
  if (onFile.length > 0) {
    ...
    for (const i of full) parts.push(`- ${what(i)}: ${i.answer}  [${i.source}]`);
    if (part.length > 0) {
      parts.push(`Partly on file — ask ONLY for the missing part, citing what's on file:`);
      for (const i of part) parts.push(`- ${what(i)}: ${i.answer}  [${i.source}] — still missing: ${i.missing}`);
    }
    parts.push(``);
  }
