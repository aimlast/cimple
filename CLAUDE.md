...
- [ ] **BIG BATCH (founder, 2026-09-25) — in progress.** The founder's list, plus the CIM-PRO items they asked me to add:
  1. **Buyer profile page** — a real page per buyer (not only a side sheet): everything collected about the buyer, the source of each field (buyer themselves / NDA / CRM / CSV / broker edit), fully editable by the broker (broker edits live in a broker-private overlay, never over a self-registered buyer's own profile); per-buyer activity timeline (access, views, NDA answers, decisions, questions, outreach), AI summary, interest tag, "email this buyer" (broker clicks send). *(timeline/AI summary/interest tag/email = CIM-PRO extras)*
  2. **Demo buyers** — ~80 complete, diverse fake buyer profiles (individuals, search funds, strategics, PE, family offices, independent sponsors; $100K → $50M deal sizes; Canada + US; `.invalid` emails; mixed sources) in the founder's account.
  3. **Clean up deals** — founder's account `broker_demo`: KEEP Amlin, SariKnotSari, 180 Smoke Vape (real deals with real documents — may be migrated/altered, never their source documents); ARCHIVE (restorable) every other old deal. Then a clean set of example deals across industries and at every stage of the CIM process, with fake source material: compilation-statement financials, T2s, leases, minute books, AR aging, employee lists, equipment lists, email threads, call transcripts, video-call transcripts, CRM notes — a made-up broker persona (founder's choice) and fake sellers; fictional brokerage brand.
  4. **Deal list** — replace the unclear "Needs action / Waiting" split with broker-chosen sort / group / filter (most recent, phase, industry, status, activity…); archive + "show archived".
  5. **Collected information always accessible** — a deal "Information" tab that never disappears (e.g. SariKnotSari after the interview is done): every fact grouped by CIM section with its exact source (which document, seller interview, email, call, video call, CRM, website, social media, broker edit), editable/addable/removable by the broker, filterable by source, conflicts/alternates visible. Consistent deal interface across phases.
  6. **CIM builder** — add / delete / duplicate / reorder / edit sections easily (CIM-PRO-style), AI rewrite per section with instructions (tone, length) *(CIM-PRO AI writer)*, per-section access tier (teaser vs full; locked sections shown as locked) *(CIM-PRO tiers)*.
  7. **Branding** — brokerage/firm branding + the company-being-sold branding (logo, colours, photos) in CIMs, editable; company branding never in the Blind CIM.
  8. **Design templates** — several CIM design templates; brokerages choose one, create/edit their own, set a default; brokerage branding applied across templates; "match my existing CIM" — upload a past CIM and Cimple follows its section structure *(CIM-PRO onboarding trick)*; print-friendly version / print preview *(CIM-PRO; link-based viewing stays the default — export decision still TBD)*.
  9. **Interactive CIM blocks** — Google Maps location, videos (YouTube/Vimeo/upload), photo galleries/images (blind-safe rules).
  10. **CRM scrub, both sides** — buyers: CRM → Cimple buyer profile fields, visible with source; sellers: link a Cimple deal to the CRM deal/organisation/person and pull fields, notes, emails, files into the deal's information with provenance "CRM"/"email".
  11. **Provenance everywhere** — every fact shows where it came from (CRM, email, call, video call, interview, document, website, social media, broker).
