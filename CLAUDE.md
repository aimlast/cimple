...
- [ ] **Founder's next-feature list (2026-09-21, in suggested build order):**
  ...
  5. **Broker-led / joint interview — decided 2026-09-22, in progress.** "Interview together" on the deal, two ways:
     ...
     - In both: the AI extracts facts from the live transcript, updates coverage + readiness, queues the next question; broker can skip/rephrase/type notes. Founder's stated pain: old-school brokers won't juggle two platforms or remember to click "listen" — so no separate listen step. Vendor keys needed from the founder: `DAILY_API_KEY`, `RECALL_API_KEY` (Railway env). Build order: (1) mode + broker question panel + floating window + in-person mode (no keys), (2) in-Cimple call + transcription, (3) bot for external platforms.
