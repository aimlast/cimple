...
- [ ] **QA harvest from demo seeding — NEXT fix batch** (full list: `docs/qa/2026-09-26-demo-seeding-qa-harvest.md`, ~120 observations from running the real app end to end). Themes: (1) Interview (crown jewel): false stop signal ends interviews early (turn-guard STOP_PHRASES), filler/praise guard misses whole-sentence grading/praise (8/15 replies on one deal), re-asks facts already in documents/transcripts and across sessions, ignores seller retractions, closes while critical sections are partial, doesn't probe planted conflicts, seller-offered documents become notes not upload requests. (2) Fact accuracy: SOURCE_RANK lets a call beat financial documents, same-rank "first/last wins" regardless of fiscal year/recency, placeholders recorded as values, revenueByYear merge ignores rank, SDE extracted as revenue, extractor computes derived metrics with arithmetic errors, industry replaced by NAICS text, resolved discrepancies don't write back for AI-invented field names, discrepancy engine flags identical values, financial analysis adds dividends as owner comp (contradicts the rule), headline figures from broker-only CRM leads, quality score inflated early. (3) CIM: generation not gated on a discrepancy check, writer double-counts owner comp in SDE / invents FY columns / partial debt, blind "[Province/State]" placeholders left, rendering bugs (numbers wrapping mid-digit in highlight cards, clipped donut legend, financial summary revenue row hidden as header). (4) Misc: source dates one day early (UTC), source counts disagree across screens, Overview onboarding struck-through yet "Waiting on seller", match-run NaN, seller tokens in request logs (security).
