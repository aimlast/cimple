...
// ── Integration (iguards × facts1/findisc): a retraction next to the merge's corroborations and discrepancies ──
{
  // The interview holds the value and a document said the very same thing
  // (a corroboration): withdrawing the guess keeps the value on the document's word.
  const info: Record<string, unknown> = {
    warrantyTerms: "2-year parts and labour",
    _fieldSources: { warrantyTerms: { source: "interview", sessionId: "s1", turn: 2 } },
    _fieldCorroborations: { warrantyTerms: [{ source: "document", documentId: "doc-payroll", value: "2-year parts and labour", at: "2026-09-01T00:00:00Z" }] },
  };
  const r = applySellerRetractions(info, [{ field: "warrantyTerms", reason: "guess" }], { turn: 4 });
  assert.deepEqual(r.restoredFromDocument, ["warrantyTerms"]);
  assert.equal(info.warrantyTerms, "2-year parts and labour");
  assert.equal((info._fieldSources as any).warrantyTerms.documentId, "doc-payroll");
  assert.equal(info._fieldCorroborations, undefined, "the document is the recorded source now, not a corroboration");

  // Only a CRM lead said the same: the value goes, the lead is kept as an alternate.
  const info2: Record<string, unknown> = {
    warrantyTerms: "2-year parts and labour",
    _fieldSources: { warrantyTerms: { source: "interview", sessionId: "s1", turn: 2 } },
    _fieldCorroborations: { warrantyTerms: [{ source: "crm", documentId: "doc-crm", value: "2-year parts and labour" }] },
  };
  const r2 = applySellerRetractions(info2, [{ field: "warrantyTerms", reason: "guess" }], { turn: 4 });
  assert.deepEqual(r2.removed, ["warrantyTerms"]);
  assert.equal(info2.warrantyTerms, undefined);
  assert.equal(info2._fieldCorroborations, undefined);
  assert.equal((info2._fieldAlternates as any).warrantyTerms[0].source, "crm");

  // Open discrepancies whose seller side is the withdrawn value are settled.
  const withdrawn = [
    { key: "annualRevenue", value: "$2.3M", turn: 5 },
    { key: "revenueByYear", value: JSON.stringify({ "2024": "$2,300,000" }), turn: 5 },
  ];
  const rows = [
    { id: "a", status: "open", factKey: "annualRevenue", interviewValue: "$2.3 million", sideSources: { interview: { kind: "interview" } } },
    { id: "b", status: "open", factKey: "revenueByYear", factYear: "2024", interviewValue: "$2.3M", sideSources: { interview: { kind: "call" } } },
    { id: "c", status: "open", factKey: "revenueByYear", factYear: "2023", interviewValue: "$2.1M" }, // another year
    { id: "d", status: "resolved", factKey: "annualRevenue", interviewValue: "$2.3M" }, // the broker's decision stands
    { id: "e", status: "open", factKey: "annualRevenue", interviewValue: "$2.3M", sideSources: { interview: { kind: "questionnaire" } } }, // typed, not withdrawn
    { id: "f", status: "open", factKey: "annualRevenue", interviewValue: "$1.9M" }, // a different figure
    { id: "g", status: "ask_seller", factKey: "sdeByYear", interviewValue: "$2.3M" }, // another fact
  ];
  assert.deepEqual(discrepanciesSettledByRetraction(rows, withdrawn), ["a", "b"]);

  // Speakers survive facts1's extraction structuring and merge: a call's
  // _speakers map reaches the recorded source (incl. a figure moved to its
  // own measure's key — "adj EBITDA" under ebitda becomes adjustedEbitda).
  const extraction = normaliseExtraction({
    brakeCondition: "110-ton brake rebuilt in 2023",
    ebitda: "Adjusted EBITDA about $1.4M",
    _speakers: { brakeCondition: "Luis Ortega (operations manager)", ebitda: "Gord Halvorsen (seller)" },
  });
  assert.deepEqual(extraction._speakers, {
    brakeCondition: "Luis Ortega (operations manager)",
    ebitda: "Gord Halvorsen (seller)",
    adjustedEbitda: "Gord Halvorsen (seller)",
  });
  const merged = mergeExtractedData({}, extraction, { documentId: "teams-9", source: "video_call" });
  assert.equal(merged._speakers, undefined, "the speaker map is never a fact");
  recordFactSpeakers(merged, extraction._speakers, "teams-9");
  assert.equal((merged._fieldSources as any).brakeCondition.speaker, "Luis Ortega (operations manager)");
  assert.equal((merged._fieldSources as any).adjustedEbitda?.speaker, "Gord Halvorsen (seller)");
  ok("integration: a document's same-value corroboration survives a retraction; conflicts about the withdrawn value are settled");
}
