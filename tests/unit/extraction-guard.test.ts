...
(async () => {
  ...
  // ── 2. Beacon FY2024: the statement never says SDE / EBITDA / add-back → none survive; line items stay ──
  {
    ...
    for (const k of ["sde2023", "sde2024", "ebitda2023", "ebitda2024", "addbacks2024", "workingCapital2024", "grossProfitMargin2024",
      "sde", "ebitda", "sdeByYear", "ebitdaByYear", "addbacksByYear", "workingCapitalByYear", "grossProfitMarginByYear"]) {
      assert.equal(out[k], undefined, `${k} dropped`);
    }
    // Line items stay — on their by-year maps (facts1's structure), the latest year as the headline.
    const year = (k: string) => (out[k] as Record<string, string> | undefined)?.["2024"];
    assert.equal(year("netIncomeByYear"), "$496,728");
    assert.equal(out.netIncome, "$496,728");
    assert.equal(year("grossProfitByYear"), "$3,022,100");
    assert.equal(year("amortizationByYear"), "$72,740");
    assert.equal(year("managementSalaryShareholderByYear"), "$185,000");
  ...
  {
    // null = no source text on file (a stored extraction replayed): no derived figure can be checked.
    const fy23 = normaliseExtraction(BEACON_FY2023, null) as Record<string, unknown>;
    assert.equal(fy23.sde, undefined);
    assert.equal(fy23.ebitda, undefined);
    assert.equal(fy23.sdeByYear, undefined, "the calculated SDE's year is not kept on a map either");
    assert.equal(fy23.ebitdaByYear, undefined);
    assert.equal(fy23.netIncome, "$414,656");
    assert.deepEqual(fy23.netIncomeByYear, { "2023": "$414,656", "2022": "$358,236" });
    ...
    assert.equal(t2.ownerSalaryByYear, undefined);
    assert.deepEqual(t2.salariesAndWagesByYear, { "2023": "$1,442,300" });
  ...
  // ── 4. Clearwater: calculated SDE / EBITDA dropped, the report's printed EBITDA kept and flagged ──
  {
    ...
    assert.equal(fy23.sde, undefined);
    assert.equal(fy23.ebitda, undefined);
    assert.equal(fy23.netIncome, "$243,122");
    assert.equal(fy23.sdeByYear, undefined);
    assert.equal(fy23.ebitdaByYear, undefined);
    ...
    assert.equal(fy24.sdeByYear, undefined);
    assert.equal(fy24.ebitdaByYear, undefined);
    assert.deepEqual(fy24.interestExpenseByYear, { "2024": "$14,300" });
    ...
    assert.equal(report.ebitda, "$426,100", "a printed EBITDA is kept (its year becomes the period)");
    assert.deepEqual(report.ebitdaByYear, { "2024": "$426,100" });
    assert.deepEqual(report.hillhurstEbitdaByYear, { "2024": "$449,851" }, "a printed location EBITDA is kept");
    ...
    assert.equal(merged.ebitda, "$426,100");
    assert.equal(getFieldSources(merged).ebitda?.note, STATED_METRIC_NOTE);
    assert.equal(getFieldSources(merged).hillhurstEbitdaByYear?.note, STATED_METRIC_NOTE, "a stated year map is flagged too");
